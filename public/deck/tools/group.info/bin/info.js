...
// Preparation
//
var $R = 'tools/group.info';
var $T = $R + '/templates';

var urlList = {
    main:        $T + '/main.tpl'
  , summary:     $T + '/summary.tpl'
  , member:      $T + '/member.tpl'
  , groups:      $T + '/groups.tpl'
  , tags:        $T + '/tags.tpl'
...
var getData = function( callback ){
  ...
  if( lias.debugMode === true )
    ...
    url = '/-/profile-data.php?groupId=' + id.substring( 1 );
...
var TemplatesView = Backbone.View.extend({
     ...
     *  }
    */
    getSummarySetting: function(){
      return {
        'summary': $Inner.getInput( this.dialogNS + 'summaryText' ).val()
      };
    }
  ...
  , getBasicInfoSetting: function(){
      ...
      settings = {
          founded: $Inner.getInput( this.dialogNS + 'founded' ).val()
      };
...
// For editor
route( 'tlg11', 'edit', function(){
  Backbone.history.navigate( $Util.currentHash.getRoot() + '/', { trigger: true, replace: true } );
});


// For setting section
route( 'tlg11', 'edit/:section', function( section ){
  var url = urlList[ section ];
  if( !url ){
    $Error.notice( 'no_tool_option', 'Lias::Tools::GroupInfo - Tool Option Not Found' );
    return false;
  }
  ...
  var applyData = function(){
    getData( function( data ){
      if( data.permission !== true ){
        $Error.notice( 'invalid_command', 'Lias::Tools::GroupInfo - Invalid Command' );
        return false;
      }
