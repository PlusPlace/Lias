/**
 *  Lias::Tools::PersonalDirectMessage
 *
 *  @identity   tlp5
 *  @version    0.0.1
...
// Preparation
//
var $R = 'tools/personal.direct-message';
var $T = $R + '/templates';
var urlList = {
    main: $T + '/main.tpl'
  , viewer: $T + '/viewer.tpl'
...
var TemplatesView = Backbone.View.extend({
    ...
    */
    addMessage: function( type, message, year, month, day, hour, minute ){
      ...
      $( '#personal-dm-inner-body' ).append( html );
      $( '#personal-dm-body' ).tinyscrollbar_update( 'bottom' );
    }
...
Backbone.history.routeTool( 'tlp5', '/', function(){
...
Backbone.history.routeTool( 'tlp5', 'destination/:destination', function( destination ){
  var applyData = function(){
    var data = {
    ...
    var update = function(){
      try{
        $( '#personal-dm-body' ).tinyscrollbar_update( 'relative' );
      } catch( e ){
        $( window ).unbind( 'resize', update );
      }
    };

    $( '#personal-dm-body' ).tinyscrollbar( 'bottom' );
    ...
    $Inner.getButton( 'personal-dm-send-button' ).bind( 'click', function(){
      var textarea = $Inner.getInput( 'personal-dm-textarea' );
